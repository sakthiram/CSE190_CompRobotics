...
def move (p_curr, motion, p_move):
    ...
    num_rows = len(p_curr)
    #print "num cols, num rows", num_cols, num_rows
    for i in range(num_rows):
        for j in range(num_cols):
            #print i, j
            from_posx = (i - motion[0] )% num_rows
            from_posy = (j - motion[1] )% num_cols
            #print from_posx, from_posy
            #print p_curr[i][j]
            #print p_curr[int(from_posx)][int(from_posy)]
            q[i][j] = p_move * p_curr[from_posx][from_posy] + (1-p_move) * p_curr[i][j]
    #print " posterior after motion:"
    #show (q)
    alpha = 0.0        
...
def sense(p_curr, sensor_data, p_correct, colors):
    q = p_curr
    num_cols = len(p_curr[0])
    num_rows = len(p_curr)
    for i in range(num_rows):
        for j in range(num_cols):
            hit= colors[i][j]==sensor_data
